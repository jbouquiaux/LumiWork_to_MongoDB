...
def scf_inp(structure):
    pseudodir='/gpfs/home/acad/ucl-modl/jbouq/paw-sr-11_pbe_standard_xml'
...
def relax_kwargs():
    ...
    relax_kwargs = dict(
        ...
        ionmov=2,
        dilatmx=1.15,  # Keep this also for optcell 0 else relaxation goes bananas due to low ecut.
        chkdilatmx=0,
    )
...
def build_flow(options):
    ...
    stru_prim=structure.Structure.from_file("SALON.cif")
    doped_stru_list=make_doped_supercell(prim_structure=stru_prim,supercell_size=[1,1,3],replaced_atom='Sr',dopant_atom='Eu')
    
    ####### Delta SCF part of the flow #######
    
    from abipy.flowtk.lumi_works import LumiWork

    for stru in doped_stru_list:
